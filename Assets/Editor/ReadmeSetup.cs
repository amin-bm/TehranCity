...
/// <summary>
/// فاز A / قدم A10 — تولید README کنترل‌ها + خلاصه فاز A.
/// idempotent: اگر فایل موجود باشد، timestamp به‌روز می‌شود.
/// </summary>
public static class ReadmeSetup
{
    private const string ReadmePath = "Assets/_Project/README_CONTROLS.md";

    [MenuItem("TehranCity/Setup/30 Generate README (Controls + Phase A Summary)")]
    public static void Run()
    {
        EnsureFolder(Path.GetDirectoryName(ReadmePath));
        string content = GenerateReadme();
        File.WriteAllText(ReadmePath, content, System.Text.Encoding.UTF8);
        AssetDatabase.Refresh();
        Debug.Log($"[TehranCity] README generated at {ReadmePath}. فاز A کامل شد.");
    }

    private static string GenerateReadme()
    {
        return $@"# Tehran City — کنترل‌ها و خلاصه فاز A

## وضعیت فاز A (Art Pass + Audio + Lighting + Post Processing)
**تاریخ:** {System.DateTime.Now:yyyy-MM-dd}
